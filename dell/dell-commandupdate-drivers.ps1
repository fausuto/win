<#
.SYNOPSIS
    This script will use Dell Command | Update to install all base drivers for
    the current system configuration on a freshly installed operating system.

.DESCRIPTION
    What this scrilpt will do is check to make sure that Dell Command | Update
    is installed. It will then check if the executable is located in its directory.
    If it is then it will be used to download and install the base drivers for
...
$TranscriptFile = "$ProvisioningPath\${Date}_dell-commandupdate-drivers.txt"
...
if ($SoftwareInstalled)
{
    ...
    if (Test-Path -Path $DcuCli)
    {
        ...
        Write-CleanLog @("", "Enabling Advance Driver Restore")
        Write-CleanLog @("", "& `"$DcuCli`" /configure -advancedDriverRestore=enable", "") -Color DarkGray

        & $DcuCli /configure -advancedDriverRestore=enable

        Write-CleanLog @("", "Installing base drivers for this workstation...")
