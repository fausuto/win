<#
.SYNOPSIS
    This script will use Dell Command | Update to scan for updates and drivers.

.DESCRIPTION
    What this scrilpt will do is check to make sure that Dell Command | Update
    is installed. It will check if the .exe is located in its directory. If so,
    it will scan the machine for any available updates or drivers to be staged
    for applying with another script.
...
$TranscriptFile = "$ProvisioningPath\${Date}_dell-commandupdate-scan.txt"
...
if ($SoftwareInstalled)
{
    ...
    if (Test-Path -Path $DcuCli)
    {
        ...
        Write-CleanLog @("", "Scanning for any available updates...")
